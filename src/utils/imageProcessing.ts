function applyFilter(img: HTMLImageElement, filterFunction: (data: Uint8ClampedArray) => void): string {
...
export function adjustBrightness(img: HTMLImageElement, brightness: number): string {
  return applyFilter(img, (data) => {
...
export function adjustContrast(img: HTMLImageElement, contrast: number): string {
  return applyFilter(img, (data) => {
...
export function sephiaFilter(img: HTMLImageElement): string {
  return applyFilter(img, (data) => {
    for (let i = 0; i < data.length; i += 4) {
...
export function negativeFilter(img: HTMLImageElement): string {
  return applyFilter(img, (data) => {
    for (let i = 0; i < data.length; i += 4) {
...
export function blackAndWhiteFilter(img: HTMLImageElement): string {
  return applyFilter(img, (data) => {
    for (let i = 0; i < data.length; i += 4) {
